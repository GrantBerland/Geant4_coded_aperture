...
MF  = 1
rec_image1 = fftconvolve(avg_image - MF*np.mean(avg_image), decoder, mode='full')
rec_image1[rec_image1 < 0] = 0

decoder[decoder == -1] = -0.7059;
decoder[decoder == -1] = -0.7059;
rec_image2 = fftconvolve(avg_image, decoder, mode='full')
...
```{python}
data = pd.read_csv("../data/pinhole_test2.csv", names=['E','det','i','j','x0','y0','z0']);
