...
data = pd.read_csv("../data/NTHT_test2.csv", names=['E','det','i','j','x0','y0','z0']);
data.info()
...
for detID in detectors:
    ...
    #avg_image += np.random.poisson(lam=100, size=[16,16]);
...
MF  = 0
mode = 'full'
rec_image1 = fftconvolve(avg_image - MF*np.mean(avg_image), decoder, mode=mode)
#rec_image1[rec_image1 < 0] = 0

#decoder[decoder == -1] = -0.6364;
#decoder[decoder == -1] = -0.1468;
rec_image2 = convolve2d(avg_image, decoder, mode=mode)
...
```{python}
data = pd.read_csv("../data/pinhole_test6.csv", names=['E','det','i','j','x0','y0','z0']);
data.info()
...
for detID in detectors:
    sig = np.histogram2d(data.i[data.det == detID].values, data.j[data.det == detID].values, 
                        bins=[np.linspace(0,16,17),np.linspace(0,16,17)]);
    #plt.figure(figsize=(14,5));plt.subplot(1,2,1);c=plt.imshow(sig[0], cmap='Blues'); plt.colorbar(c); plt.title("Detector %i" % detID);
    avg_image += sig[0];
    avg_image += np.random.poisson(lam=bg_noise_level/len(detectors), size=[16,16]);
    
plt.figure(figsize=(14,5)); plt.subplot(1,2,1); plt.title("Raw Detector Image")
c=plt.imshow(avg_image, cmap='Blues'); plt.colorbar(c);
```
